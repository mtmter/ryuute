...
画面幅が721px以上の月表示と週表示では、ミニカレンダーと準備案内を同じ左サイドバー内へ縦に並べます。
...
月表示と週表示で目的の日付へ移動しやすくするため、既存のメインカレンダーの左側に補助的なミニカレンダーを追加します。既存のヘッダー、月カレンダー、週カレンダー、タスク表示、各モーダルは置き換えません。
...
現在の日付は常に識別できるようにします。月表示ではメインカレンダーの表示基準日を選択日として強調します。週表示ではメインカレンダーに表示している7日間を強調し、その中の表示基準日をさらに識別できるようにします。現在の日付と選択日が同じ場合も、両方の状態が分かる表示にします。
...
1. 画面幅が721px以上の月表示と週表示で、既存カレンダーの左側にミニカレンダーと準備案内のサイドバーが表示される
...
11. ミニカレンダーの日付を選択しても予定追加モーダルが開かない
12. 既存ヘッダーによる日付移動がミニカレンダーにも反映される
13. 表示位置の変更のためのAPI通信、DB変更、外部サービス連携が発生しない
14. 既存の月表示、週表示、タスク表示、予定・タスク操作、準備案内、経路検索が従来どおり動作する
